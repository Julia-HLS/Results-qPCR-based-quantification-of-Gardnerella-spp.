...
mastertable <- read_excel("2025_mastertable_adjustedFemale.xlsx", 
...
png("Plots/adjustedFemale_Gspp_load_frequency_sex_bp.png", width = 2700, height = 1200, res = 300)
...
Copies_Frequency_plot <- ggplot(mastertable, aes(x = frequency_grouped, y = GV_log_copies_swab, fill = Body_site)) +
  ...
  theme_classic() +
  labs(
    title = expression(italic("Gardnerella spp.") * " DNA copies in relation to frequency of sexual intercourse - adjusted female"),
...
addWorksheet(workbook, "Dunn_results_adjustedFemale")
writeData(workbook, "Dunn_results_adjustedFemale", results_combined)

# save to Excel file
saveWorkbook(workbook, "Plots/DunnTest_results_adjustedFemale.xlsx", overwrite = TRUE)