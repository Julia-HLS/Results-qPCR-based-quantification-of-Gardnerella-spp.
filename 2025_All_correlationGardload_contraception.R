...
png("Plots/All_Gspp_load_vaginal_penile_protection.png", width = 2700, height = 1200, res = 300)

# Plot
ggplot(paired_data, aes(x = Vaginal, y = `Penile skin`)) +
  geom_point(aes(shape = protection), color = "violet", size = 4) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  geom_text(aes(label = couple_no), vjust = -0.8, size = 3.5, fontface = "bold") +
  annotate("text",
  ...
  scale_shape_manual(
    values = c("yes" = 16, "no" = 17, "not_available" = 8),
    name = "Protection"
  ) +
  
  # Axis titles
  labs(
    title = expression(italic("Gardnerella spp.") * " load: vaginal and penile within couples"),
