...
png("Plots/Gspp_load_frequency_sex_bp.png", width = 2700, height = 1200, res = 300)

# Create Plot
Copies_Frequency_plot <- ggplot(mastertable, aes(x = frequency_grouped, y = GV_log_copies_swab, fill = Body_site)) +
  geom_boxplot(position = position_dodge(width = 0.8),
               outlier.shape = NA, color = "black") +  # narrower boxplots
  ...
  theme_classic() +
  labs(
    title = expression(italic("Gardnerella spp.") * " DNA copies in relation to frequency of sexual intercourse"),
...
Copies_Frequency_plot +
  ...
  annotate("text", x = 1, y = 9.5, label = p_vag, color = "palevioletred", size = 4.5, fontface = "bold") +
  annotate("text", x = 1, y = 9, label = p_pen, color = "paleturquoise4", size = 4.5, fontface = "bold") +
  theme(
...
# Table with p-values for each categorical comparison

# Dunn-Test with BH-correction for both all body sites
...
addWorksheet(workbook, "Dunn_results")
writeData(workbook, "Dunn_results", results_combined)

# save to Excel file
saveWorkbook(workbook, "Plots/DunnTest_results.xlsx", overwrite = TRUE)